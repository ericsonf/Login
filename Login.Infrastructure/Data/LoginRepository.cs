...
namespace Login.Infrastructure.Data
{
    public class LoginRepository : IRepository
    {
        ...
        public PagedResult<T> List<T>(string[] navProperties, Expression<Func<T, Object>> predicateSort, Enum typeOrder, int page, int limit) where T : BaseEntity
        {
            IQueryable<T> result = _dbContext.Set<T>();
            result = SetIncludes(result, navProperties);
        ...
        public PagedResult<T> List<T>(Expression<Func<T, Object>> predicateSort, Enum typeOrder, int page, int limit) where T : BaseEntity
        {
            IQueryable<T> result = _dbContext.Set<T>();
            result = GetOrderResult(result, predicateSort, typeOrder.ToString());
        ...
        public PagedResult<T> Filter<T>(Expression<Func<T, bool>> predicate, string[] navProperties, Expression<Func<T, Object>> predicateSort, Enum typeOrder, int page, int limit) where T : BaseEntity
        {
            if (predicate == null) return null;
            IQueryable<T> result = _dbContext.Set<T>().Where(predicate);        
            result = SetIncludes(result, navProperties);
        ...
        public PagedResult<T> Filter<T>(Expression<Func<T, bool>> predicate, Expression<Func<T, Object>> predicateSort, Enum typeOrder, int page, int limit) where T : BaseEntity
        {
            if (predicate == null) return null;
            IQueryable<T> result = _dbContext.Set<T>().Where(predicate);
            result = GetOrderResult(result, predicateSort, typeOrder.ToString());
